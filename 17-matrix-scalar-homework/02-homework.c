 ...
 * impiegando la 2 strategia per la distribuzione del dominio 2D 
 * (suddivisione del lavoro per colonne)
*/
...
int main() {
    ...
    #pragma omp parallel for shared(matrix) private(i,j) collapse(2)
    for ( j = 0; j < M; j++ ) {
        for ( i = 0; i < N; i++ ) {
